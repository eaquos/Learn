...
"""
def scatterplot(x,y):
    pyplot.plot(x,y,'b.')           #read this for futher information http://matplotlib.org/api/pyplot_api.html#matplotlib.pyplot.plot
    pyplot.xlim(min(x)-1,max(x)+1)  #setup the X axis limit
    pyplot.ylim(min(y)-1,max(y)+1)  #setup the Y axis limit
